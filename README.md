...
Как я уже написал выше, у расширения есть несколько режимов, но у самого Тора тоже есть режимы работы: pro и default. В режиме default выходной ip подбирается случайно, а самим Тором используется 10 мостов одновременно (и скорее всего в любой конкретный момент не все из них работают). Это значительно минимизирует шанс отказа какой-либо части системы, а Тор состоит из трёх частей: мосты, миддлы (они справляются с задачей и работают без перерыва больше года, а потому не рандомятся), и выходы. Большой проблемой Тора является тот факт, что все его выходы принципиально открыты для чужих глаз, рабочих из них менее двух тысяч.

В режиме pro выходной ip задан заранее, по принципу, что он является достаточно быстрым, стабильным и обходит блокировки Ютуба и Дискорда на Тор, хотя эти задачи в целом несколько противоречат друг другу, ибо такие блокировки действуют на наиболее популярные, а значит наиболее быстрые выходы. Например, имелись случаи, когда в default Дискорд запрещал отправлять изображения, а в pro всё работало корректно. Его минусом является возможность массового отказа для всех тех, у кого этот режим стоял; по сравнению с ним режим default является практически вечным. В случае отказа pro необходимо или переключиться на default, или заменить вышедший из строя элемент. Мостов в режиме pro два, а не десять, ибо фундаментально, больше для работы Тору не требуется; это также существенно влияет на время первого запуска в лучшую сторону.
