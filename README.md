...
Основных режимов три — System, AUTO и VIRTUAL. Их вы можете переключать нажатием кнопки мыши по расширению, вам необходимо сделать это после синхронизации (лучше остановиться на AUTO). System используется для совместимости со сторонними VPN, полностью отключая проксирование. Выглядит как чёрный кружок с белым центром; именно он стоит первым. Режим AUTO — фиолетовый круг с чёрным центром, по умолчанию использует устойчивую сеть PAC прокси-серверов [Антизапрет](https://antizapret.prostovpn.org/) для полного раздельного туннелирования только заблокированных сайтов, задействуя Тор по отношению к редактируемому списку, в котором содержатся сайты, заблокировавшие себя от России самостоятельно — [no-russia-hosts](https://github.com/dartraiden/no-russia-hosts). Антизапрет работает только для россиян, так как содержит список сайтов, заблокированных в России.

![2](https://github.com/user-attachments/assets/4029c6f7-c63a-44b9-9594-ea8508708689)

Если вы добавили свои сайты в режим AUTO, и сделали бэкап, вы должны проследить за тем, чтобы конечный файл весил не больше 10 мегабайт: это лимит, при котором он корректно читается. [Антицензорити](https://github.com/anticensority/generated-pac-scripts) — очень жирный список, весящий более 9 мегабайт и по умолчанию даже не использующийся, в своих файлах синхронизации я оставляю только ссылку на него, уменьшая вес .bak в 10 раз, но у вас он может скачаться в фоне и сломать конфиг. Более подробно об Антицензорити написано в конце инструкции.

VIRTUAL принимает значения любого другого режима и проксирует весь браузерный трафик, выглядит как полностью фиолетовый круг. В VIRTUAL возможно измерить скорость подключения, используйте для этого удобный вам сайт, например speedcheck.org. Вашей скорости должно с запасом хватать на 1080 60, на 2К 60. VIRTUAL также используется для скрытия ip-адреса, но так как корпорации добра Гугл плевать на приватность своих пользователей (в отличие от Firefox), присутствует утечка на абсолютно любой странице без вашего ведома. Чтобы этого избежать, необходимо установить расширение [WebRTC Leak Shield](https://chromewebstore.google.com/detail/webrtc-leak-shield/bppamachkoflopbagkdoflbgfjflfnfl), проверяйте наличие утечек на [WebRTC Leak Test](https://browserleaks.com/webrtc). Это относится ко всем хромоподобным браузерам, включая Edge и Opera.
...
На некоторых сайтах (например polymarket.com) могут быть сложности с регистрацией даже в режиме VIRTUAL на PROXY, так как у него слишком много защит. Важно знать, что Тор не всесилен, хотя обеспечивает свободный доступ к 99% интернета. Это очередное напоминание, какую ошибку совершили разработчики, открыв все торовские адреса для публичного доступа.
