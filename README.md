...
Select the Tor you need and unpack it. Avoid spaces and special characters in the paths to your folder, use full-stop to exit Tor on Linux. AntiDetect does not have an exe file without a digital signature, so it does not provoke antiviruses. Other options for Windows can provoke a false antivirus reaction, but have an exe. Running AntiTor will open a console with connection information, while running service-create/manager (Linux/Windows) will open Tor silently. To remove the service, run service-* again. AntiTor and service-* are the only two files that will run Tor correctly. Tor will start with all the necessary universal connection settings.
