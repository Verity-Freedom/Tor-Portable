...
В режиме pro выходной ip задан заранее, по принципу, что он является достаточно быстрым, стабильным и обходит блокировки на Тор. Например, имелось множество случаев, когда в default Дискорд запрещал отправлять изображения, а в pro всё работало корректно. Мост в режиме pro один, а не пять, ибо больше для работы Тору не требуется, а их большее количество может способствовать появлению ошибок и снижению скорости, если не все они работают. Его минусом является возможность массового отказа; по сравнению с ним default является практически вечным (может как перестать работать полностью, так и ухудшиться функционал; изображения в Дискорде перестают отправляться, блокировки Ютуба тригерятся). В случае отказа pro необходимо или переключиться на default, или скачать новые архивы режимов через updater, или перекачать Тор целиком.
