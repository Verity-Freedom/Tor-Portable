...
Откройте папку drover, запустите файл drover.exe и нажмите install. Это разблокирует клиентское приложение Дискорда вместе с голосовыми чатами без всяких задержек благодаря обходу UDP. Начиная с этого момента для корректной работы Тор должен быть или службой, или запущен как программа.
