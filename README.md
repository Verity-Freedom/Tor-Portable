...
The Tor Project develops breakthrough technologies for bypassing censorship. However, their flexibility is constrained by several factors: 1) excessive dependence on the browser, which always requires a connection to the Tor network; 2) the lack of exits hidden from external censorship. By freeing the Tor core from the browser, I return to this technology the freedom that it should retain, and I always say that Tor is not a browser. Binding to a browser prevents maintaining support for Windows 7. [Another issue mentioned has been acknowledged by the developers.](https://gitlab.torproject.org/tpo/core/tor/-/issues/41098). The Russian community [used](https://ntc.party/t/мосты-webtunnel-в-tor-browser/15595/150) a patched, censorship-resistant version of Webtunnel several months before the developers officially introduced similar changes (although the official version is safer and so significantly better, but hundreds of people benefited from the quick patch). The core itself is much lighter than the browser.
